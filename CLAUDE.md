...
- Per-app right-click "Move to..." (a `menu::Item::Folder`) and "Hide from menu" go through `Applet::move_targets` (categories offered, rebuilt in `Applet::rebuild_context_menus`/`rebuild_app_context_menu` alongside the cached per-app menus) since `ContextMenuAction` must be `Copy` and so carries an index into it, not an `ApplicationCategory`.
